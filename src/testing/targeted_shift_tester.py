...
def _extract_waypoint_status_from_csv(csv_path: str) -> dict:
    """
    Extract waypoint status from the rich CSV to determine when agents reach waypoints.
    
    Args:
        csv_path: Path to the rich trajectory CSV
        
    Returns:
        Dict mapping agent_id -> {step_idx: reached_status}
    """
    try:
        import pandas as pd
        ...
        # Group by agent and track waypoint reached status
        for agent_id, agent_df in df.groupby('agent_id'):
            agent_df = agent_df.sort_values('step_idx')  # Ensure proper order
            agent_status = {}
            reached = False
            
            for _, row in agent_df.iterrows():
                step_idx = int(row['step_idx'])
                # Once waypoint_reached becomes 1, agent is done for all future steps
                if not reached and row.get('waypoint_reached', 0) == 1:
                    reached = True
                    print(f"DEBUG: Agent {agent_id} reached waypoint at step {step_idx}")
                agent_status[step_idx] = reached
                
            waypoint_status[agent_id] = agent_status
            
        # Debug output
        total_reached_steps = sum(sum(agent_status.values()) for agent_status in waypoint_status.values())
        print(f"DEBUG: Extracted waypoint status for {len(waypoint_status)} agents")
        print(f"DEBUG: Total steps with reached waypoints: {total_reached_steps}")
        
        return waypoint_status
        
    except Exception as e:
        print(f"Warning: Failed to extract waypoint status from {csv_path}: {e}")
        return {}


def _merge_series_with_rich_csv(rich_csv_path: str, detector_series: dict) -> str:
    """
    Merge hallucination detector series with rich trajectory CSV data.
    
    Args:
        rich_csv_path: Path to the rich trajectory CSV generated by environment
    ...
    try:
        ...
        if df.empty or not detector_series:
            return rich_csv_path
            
        # Create series DataFrame
        series_df = pd.DataFrame({
            "step_idx": np.arange(len(detector_series["gt_conflict"])),
            "predicted_alert": detector_series["alert"],
            "tp": detector_series["tp"], 
            "fp": detector_series["fp"], 
            "fn": detector_series["fn"], 
            "tn": detector_series["tn"],
        })
        
        # Merge with rich data on step_idx 
        merged_df = df.merge(series_df, on="step_idx", how="left")
        
        # Fill NaN values for missing steps
        for col in ["predicted_alert", "tp", "fp", "fn", "tn"]:
            merged_df[col] = merged_df[col].fillna(0).astype(int)
        
        # Save merged CSV
        merged_path = rich_csv_path.replace('.csv', '_merged.csv')
        merged_df.to_csv(merged_path, index=False)
        
        return merged_path
        
    except Exception as e:
        print(f"Warning: Failed to merge series with rich CSV {rich_csv_path}: {e}")
        return rich_csv_path
...
def run_targeted_shift_grid(repo_root: str,
    ...
    # Initialize detector with proper action period (10s to match environment timestep)
    detector = HallucinationDetector(action_period_s=10.0, res_window_s=60.0, horizon_s=300.0)
    summary_rows = []
    ...
    for test_id, agent_id, shift_type, shift_value, description in targeted_shifts:
        for ep_idx, seed in enumerate(seeds):
            ...
            # Find the rich trajectory CSV file generated by the environment
            rich_csv_files = [f for f in os.listdir(shift_dir) if f.startswith('traj_ep_') and f.endswith('.csv')]
            csv_path = os.path.join(shift_dir, rich_csv_files[0]) if rich_csv_files else None

            # Compute hallucination metrics with waypoint awareness
            if csv_path:
                waypoint_status = _extract_waypoint_status_from_csv(csv_path)
                traj_with_waypoints = traj.copy()
                traj_with_waypoints["waypoint_status"] = waypoint_status
                cm = detector.compute(traj_with_waypoints, sep_nm=base_env_config["separation_nm"])
            else:
            ...
            if generate_viz and csv_path:
                # Extract waypoint status to exclude completed agents
                waypoint_status = _extract_waypoint_status_from_csv(csv_path)
                
                # Add waypoint status to trajectory for detector
                traj_with_waypoints = traj.copy()
                traj_with_waypoints["waypoint_status"] = waypoint_status
                
                # ask detector for step series (call compute() once with return_series=True)
                cm_full = detector.compute(traj_with_waypoints, sep_nm=base_env_config["separation_nm"], return_series=True)
                cm_series = cm_full.get("series", {})
                
                # Merge detector series with the rich CSV data
                merged_csv_path = _merge_series_with_rich_csv(csv_path, cm_series)
                if merged_csv_path:
                    _ = make_episode_visuals(shift_dir, merged_csv_path, cm_series, title=f"{description} | ep={ep_idx}")
...
Examples:
  ...
  python targeted_shift_tester.py --checkpoint models/my_model --scenario parallel --episodes 10 --algorithm SAC --viz
    ...
    parser.add_argument(
        ...
        type=str,
        default="parallel",
        help="Scenario name (without .json extension) or 'all' to run all scenarios. Default: parallel"
    )
